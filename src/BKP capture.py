...
def get_min_resolution(video_folder):
    ...
    for video_file in os.listdir(video_folder):
        if video_file.endswith(".mp4"):
            ...
            cap.release()
    return min_width, min_height if min_width != float('inf') and min_height != float('inf') else (640, 480)
...
def crop_video(input_path, output_path, target_width, target_height):
    try:
        ...
        logger.info(f"Video cropped successfully: {output_path}")
...
def process_gloss_sentence():
    global video_folder, output_folder, chatbot_response
    ...
    logger.info(f"Target video size: {min_width}x{min_height}")
