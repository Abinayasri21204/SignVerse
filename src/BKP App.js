...
const API_KEY = process.env.REACT_APP_TOGETHER_API_KEY || "b49688c1a8b81f6e2af5039126764bb90f47e3b47e6961c3007960bb42022cdb";
...
function App() {
  ...
  useEffect(() => {
    if (isCameraOn && videoRef.current) {
      console.log("Setting video stream from backend");
  ...
  useEffect(() => {
    let interval;
    if (isCameraOn) {
      console.log("Starting real-time prediction polling");
      interval = setInterval(async () => {
        try {
          const data = await getPrediction();
          if (data && data.status === 'success') {
            if (data.sentence) {
              console.log("Real-time prediction:", data.sentence);
              setMessageInput(data.sentence); // Update textbox with full sentence
            } else if (data.sign) {
              console.log("Single sign detected:", data.sign);
              setMessageInput(prev => prev ? `${prev} ${data.sign}` : data.sign); // Append single signs
            }
          } else {
            console.log("No valid prediction data:", data);
          }
        } catch (error) {
  ...
  const toggleCamera = async () => {
    if (isCameraOn) {
      ...
      await stopCamera();
      setIsCameraOn(false);
      setMessageInput(''); // Clear textbox when camera stops
    } else {
      ...
      try {
        ...
        if (data.status === 'success') {
          ...
          console.error("Backend camera start failed:", data.message);
  ...
  const resetSentence = async () => {
    try {
      ...
      if (data.status === 'success') {
        setMessageInput('');
        console.log("Sentence reset successfully");
      } else {
        console.error("Failed to reset sentence:", data.message);
      }
    } catch (error) {
      ...
      <Container>
          ...
          <InputContainer>
            {isRecording ? (
              ...
              <Box sx={{ position: 'relative', flexGrow: 1 }}>
                <LongTextField
                  fullWidth
                  placeholder="Type your message or use sign language..."
                  variant="outlined"
                  value={messageInput}
        ...
        <RightSection>
          <CameraPlaceholder>
            ...
            <IconButton onClick={toggleCamera} sx={{ position: 'absolute', bottom: 8, right: 8 }}>
              {isCameraOn ? <VideocamIcon fontSize="large" /> : <VideocamOffIcon fontSize="large" />}
            </IconButton>
          </CameraPlaceholder>
          <CapsuleButton onClick={resetSentence}>Reset Sentence</CapsuleButton>
